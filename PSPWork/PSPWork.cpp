...
namespace operations_research {
	namespace sat {
		...
		void LoadAndSolve(const std::string& file_name) {
			...
			for (int numberTasks=0; numberTasks<task_starts.size(); numberTasks++)
			{
				int du = model.Get(LowerBound(task_starts.at(numberTasks)));
				int dura = parser.tasks()[numberTasks].recipes[0].duration;
				printf("task number=%d :decisdtime=%d startTime=%d,endtime=%d \n", 
					numberTasks, 
					du, dura, du+dura
					);
			}
