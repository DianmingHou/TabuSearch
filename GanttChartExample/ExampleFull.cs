    ...
    public partial class ExampleFull : Form
    {
        ...
        private void mnuFileOpen_Click(object sender, EventArgs e)
        {
            using (var dialog = new OpenFileDialog())
            {
                ...
                if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    if (dialog.FileName.EndsWith(".mp")) {

                        _mManager = GetResultOfMultiFile(dialog.FileName);
                        if (_mManager == null)
                        {
                            MessageBox.Show("Unable to Cac the result of multi-project.", "Gantt Chart", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        else {
                            _mChart.Init(_mManager);
                            _mChart.Invalidate();
        ...
        private void mnuFileOpenMIT_Click(object sender, EventArgs e)
        {
            using (var dialog = new OpenFileDialog())
            {
                dialog.InitialDirectory = System.IO.Path.GetDirectoryName(Application.ExecutablePath);
                if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    if (dialog.FileName.EndsWith(".mp"))
                    {
                        ...
                        _mManager = GetMITResultOfMultiFile(dialog.FileName);
                        if (_mManager == null)
                        {
                            MessageBox.Show("Unable to Cac the result of multi-project.", "Gantt Chart", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
